...
## X trending integratie
- `services/news_trending_x.py` haalt live topics op via `X_API_BEARER_TOKEN` en cachet ze in-memory.
- Frequentie:
  - `X_TRENDING_CACHE_TTL_SECONDS` (default 180s) bepaalt hoe lang topics in cache blijven.
  - Wanneer de X API faalt en `X_TRENDING_ALLOW_STUBS` **niet** is gezet, de backend valt terug op de database-trendingfeed na 30 seconden retry.
  - Voor lokale demo's kan `X_TRENDING_ALLOW_STUBS=true` worden gezet om de stubtopics tijdelijk te tonen.
