...
export default function AdminEventSubmissionsPage() {
  ...
  const loadSubmissions = async () => {
    setLoading(true);
    try {
      const data = await listEventSubmissionsAdmin({
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold">Event Inzendingen</h1>
        ...
        <div className="space-y-4">
          {submissions.map((submission) => (
            <Card
              ...
              onClick={() => navigate(`/admin/event-submissions/${submission.id}`)}
            >
              <CardHeader>
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      {submission.location_text || (submission.lat && submission.lng ? `${submission.lat.toFixed(6)}, ${submission.lng.toFixed(6)}` : "Geen locatie")}
                    </p>
                    <p className="text-xs text-muted-foreground mt-1">
                      {format(new Date(submission.start_time_utc), "d MMM yyyy 'om' HH:mm", { locale: nl })}
                      {submission.end_time_utc && ` - ${format(new Date(submission.end_time_utc), "HH:mm", { locale: nl })}`}
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between text-sm text-muted-foreground">
                  <div className="flex items-center gap-4">
                    {submission.category_key && (
                      <span>Categorie: <strong>{submission.category_key}</strong></span>
                    )}
                    {submission.created_event_id && (
                      <span className="text-green-600">
                        Event ID: {submission.created_event_id}
                      </span>
                    )}
                  </div>
                  <span>
