...
import {
  getEventSubmissionAdmin,
  approveEventSubmissionAdmin,
  rejectEventSubmissionAdmin,
  unpublishEventSubmissionAdmin,
  type EventSubmissionResponse,
} from "@/lib/apiAdmin";
import { format } from "date-fns";
import { nl } from "date-fns/locale";

export default function AdminEventSubmissionDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [submission, setSubmission] = useState<EventSubmissionResponse | null>(null);
  ...
  const loadSubmission = async () => {
    ...
    setLoading(true);
    try {
      const data = await getEventSubmissionAdmin(Number(id));
      setSubmission(data);
    } catch (err: any) {
      toast.error("Kon inzending niet laden", { description: err.message });
      navigate("/admin/event-submissions");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApprove = async () => {
    ...
    setProcessing(true);
    try {
      await approveEventSubmissionAdmin(Number(id));
  ...
  const handleReject = async () => {
    ...
    setProcessing(true);
    try {
      await rejectEventSubmissionAdmin(Number(id), rejectionReason.trim() || undefined);
  ...
  const handleUnpublish = async () => {
    if (!id) return;
    if (!confirm("Weet je zeker dat je dit event wilt verwijderen? Het event zal niet meer zichtbaar zijn op de frontend.")) {
      return;
    }
    setProcessing(true);
    try {
      await unpublishEventSubmissionAdmin(Number(id));
      toast.success("Event verwijderd");
      await loadSubmission();
    } catch (err: any) {
      toast.error("Kon event niet verwijderen", { description: err.message });
    } finally {
      setProcessing(false);
    }
  };
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" onClick={() => navigate("/admin/event-submissions")}>
          <Icon name="ArrowLeft" sizeRem={1} className="mr-2" />
          Terug
        </Button>
        <h1 className="text-3xl font-bold">Event Inzending Details</h1>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Label className="text-muted-foreground">Start tijd</Label>
              <p className="font-medium">
                {format(new Date(submission.start_time_utc), "d MMM yyyy 'om' HH:mm", { locale: nl })}
              </p>
            </div>
            {submission.end_time_utc && (
              <div>
                <Label className="text-muted-foreground">Eind tijd</Label>
                <p className="font-medium">
                  {format(new Date(submission.end_time_utc), "d MMM yyyy 'om' HH:mm", { locale: nl })}
                </p>
              </div>
            )}
            {submission.location_text && (
              <div className="md:col-span-2">
                <Label className="text-muted-foreground">Locatie tekst</Label>
                <p className="font-medium">{submission.location_text}</p>
              </div>
            )}
            ...
            {submission.url && (
              <div>
                ...
                <p className="font-medium">
                  <a href={submission.url} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">
                    {submission.url}
                  </a>
                </p>
              </div>
            )}
            {submission.category_key && (
              <div>
                <Label className="text-muted-foreground">Categorie</Label>
                <p className="font-medium">{submission.category_key}</p>
              </div>
            )}
            {submission.description && (
              <div className="md:col-span-2">
                <Label className="text-muted-foreground">Beschrijving</Label>
                <p className="font-medium whitespace-pre-wrap">{submission.description}</p>
              </div>
            )}
            {submission.created_event_id && (
              <div>
                <Label className="text-muted-foreground">Aangemaakt Event ID</Label>
                <p className="font-medium text-green-600">{submission.created_event_id}</p>
              </div>
            )}
          ...
          {submission.status === "approved" && (
            <div className="flex gap-4 pt-4 border-t">
              <Button
                ...
                disabled={processing}
              >
                {processing ? "Bezig..." : "Event verwijderen"}
              </Button>
            </div>
          )}
        </CardContent>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Textarea
                ...
                placeholder="Bijv. Event bestaat al, onjuiste informatie, etc."
                rows={4}
              />
            </div>
          </div>
