...
export const DIAGNOSIS_INFO: Record<string, {
    ...
    OSM_TIMEOUT_STORM: {
        title: "Overpass API Timeout Storm",
        explanation: "Overpass API is timing out or disconnecting frequently for discovery. The system is backing off to avoid overloading the public Overpass servers. This typically indicates Overpass servers are under heavy load or network issues.",
        metrics: ["timeout_errors_last_60m", "overpass_error_count_last_60m", "overpass_calls_last_60m"],
        suggestions: [
            "Wait for Overpass servers to recover (timeouts are temporary)",
            "Check network connectivity and stability",
            "The system automatically backs off and retries with exponential backoff",
            "Consider running discovery during off-peak hours if timeouts persist",
            "Monitor Overpass API status pages for known issues",
        ],
    },
    OSM_RATE_LIMITED: {
        title: "Overpass API Rate Limited",
        explanation: "Overpass API is rate limiting discovery requests (HTTP 429). The system is slowing down and will retry later. This indicates the rate limit threshold has been exceeded.",
        metrics: ["rate_limit_errors_last_60m", "overpass_429_last_60m", "overpass_error_count_last_60m"],
        suggestions: [
            "Wait for rate limit window to reset (system respects Retry-After headers)",
            "Reduce discovery frequency or scope if rate limiting persists",
            "The system automatically backs off and respects rate limits",
            "Consider spreading discovery runs across different times of day",
        ],
    },
    OSM_SERVER_UNSTABLE: {
        title: "Overpass API Server Instability",
        explanation: "Overpass API servers are experiencing instability (5xx errors). Discovery may be degraded. This typically indicates temporary server issues on the Overpass infrastructure.",
        metrics: ["server_5xx_errors_last_60m", "overpass_error_count_last_60m", "overpass_calls_last_60m"],
        suggestions: [
            "Wait for Overpass servers to recover (5xx errors are usually temporary)",
            "Check Overpass API status pages for known outages",
            "The system automatically retries with exponential backoff",
            "Monitor worker logs for specific error messages",
            "Consider pausing discovery if instability persists for extended periods",
        ],
    },
    OSM_ERROR_RATE_HIGH: {
        title: "High OSM API Error Rate",
        explanation: "The DiscoveryBot is experiencing a high rate of errors when calling the OpenStreetMap Overpass API. This is a generic error category indicating elevated error rates that don't fit into specific timeout/rate-limit/server-error categories.",
        metrics: ["overpass_error_count_last_60m", "overpass_error_ratio_last_60m", "overpass_calls_last_60m"],
        suggestions: [
            "Check detailed error metrics to identify the specific error type",
            "Review Overpass API status and availability",
            "The system automatically backs off and retries with exponential backoff",
            "Monitor worker logs for specific error patterns",
            "Consider reducing discovery frequency if errors persist",
        ],
    },
